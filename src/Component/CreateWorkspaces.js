...
function CreateWorkspaces() {
    ...
    const {createWorkspace}=userContext;
       ...
       useEffect(() => {
        ...
        const Submitform=()=>{
        const payLoad={workspaceName:name,userId:localStorage.getItem("token")}
    ...
    <div className="Loginheader">
        ...
        <div className="LoginContainer">
            <div>
                ...
                <input onChange={handleChange} value={name}  type="text" name="name" id="name" placeholder='Name of workspace' />
                <div>
                    <button className="singupbutton" onClick={Submitform}>create</button>
                </div>
            </div>
        </div> 
        </div>
